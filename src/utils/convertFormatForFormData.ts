...
import { bytesToBase64, convertArrayToObject, convertRoundListToObject, formattingDate } from ".";
...
const convertFormatForFormData = (data: ShowFormType & ShowResFormType, main_image_color: string, position: object, roundList: DateWithTimeObj[]) => {
  const base64EncodedContents = (!!data.content && bytesToBase64(new TextEncoder().encode(data.content))) || null;
  const base64EncodedNotice = (data.method === "예매 대행" && !!data.notice && bytesToBase64(new TextEncoder().encode(data.notice))) || null;
